...
- **Notifications not firing:** Double-check TX CCCD writes (scripts abort if the enable step fails). Provide the correct 128-bit UUIDs and clear OS caches (`scripts/tools/clear_bt_cache.sh --adapter <MAC> --device <MAC>`) after firmware updates.
...
- **LE link drops immediately (Reason 0x3e)**: After forcing LE-only, a connection might still collapse right after `LE Read Remote Used Features`. Clear `/var/lib/bluetooth/<adapter>/` on both hosts with `scripts/tools/clear_bt_cache.sh --adapter <MAC> --all`, restart bluetoothd, and ensure the mock stays running—this removes stale bond info and the LE link stays up for the test.
